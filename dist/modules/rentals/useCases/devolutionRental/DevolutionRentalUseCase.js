...
let DevolutionRentalUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!rental) {
      throw new _AppError.AppError("This rental does not exists");
    }

    const dateNow = this.dateProvider.dateNow();
    let daily = this.dateProvider.compareInDays( //quantas diarias tem esse aluguel
    ...
    const delay = this.dateProvider.compareInDays( //calculando a quantidade de atrasos
    dateNow, rental.expected_return_date);
    let total = 0;

    if (delay > 0) {
      const calculate_fine = delay * car.fine_amount;
      console.log(calculate_fine); //multiplicando o numero de dias de atrasos pela diaria para que os dias atrasdos entre no total
    ...
    await this.carsRepository.updatAvailable(car.id, true);
    return rental;
  }
